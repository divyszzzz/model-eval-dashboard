...
# Column mappings exactly as in your HTML
COLUMN_MAPPINGS = {
    'qa': {
        'judge_columns': [
            'Judge_Model_A_Score',
...
def create_judge_comparison_chart(datasets, task_filter=None):
    ...
    tasks_to_show = [task_filter] if task_filter else [k for k, v in datasets.items() if v is not None]
    ...
    model_labels = [f"{model}" for model in list(MODEL_COLORS.keys())[:max_models]]
...
def calculate_best_model(datasets):
    ...
    for task, df in datasets.items():
        if df is not None and task in COLUMN_MAPPINGS:
            judge_cols = COLUMN_MAPPINGS[task]['judge_columns']
